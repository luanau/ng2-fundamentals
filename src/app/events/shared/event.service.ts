...
export class EventService {
  ...
  getEvents():Observable<IEvent[]> {
    return this.http.get(this.baseUrl+"/api/events").map((response: Response) => {
        return <IEvent[]>response.json();
    }).catch(this.handleError);
  }

  getEvent(id:number):Observable<IEvent> {
    return this.http.get(this.baseUrl+"/api/events/" + id).map((response: Response) => {
        return <IEvent>response.json();
    }).catch(this.handleError);
  }
  ...
  searchSessions(searchTerm: string) {
    return this.http.get(this.baseUrl+"/api/sessions/search?search=" + searchTerm).map((response: Response) => {
        return response.json();
    }).catch(this.handleError);
  }
